...
class ConfirmSale
{
    ...
    public function handleAnulation(Sale $sale, int $userId): Sale
    {
        if ($sale->status !== 'certified') {
            throw ValidationException::withMessages(['sale' => 'La venta no está en borrador.']);
        }
        ...
        return DB::transaction(function () use ($sale, $userId) {
            ...
            foreach ($sale->items as $item) {
                ...
                if ($sellable instanceof WeaponUnit) {
                    ...
                    if ($sellable->status !== 'SOLD') {
                        throw ValidationException::withMessages([
                            'items' => "Arma serie {$sellable->serial_number} no vendida.",
                        ]);
                    }
                    ...
                    WeaponUnitMovement::create([
                        'weapon_unit_id' => $sellable->id,
                        'type' => 'IN',
                        'reference' => 'SALE-VOID:' . $sale->id,
                        'moved_at' => now(),
                        'user_id' => $userId,
                    ]);

                    $sellable->update(['status' => 'IN_STOCK']);
                }
